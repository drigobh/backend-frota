...
console.log('🧹 Limpando duplicidades e padronizando menus e abas...');

const indexPath = path.join(__dirname, 'public', 'index.html');
let html = fs.readFileSync(indexPath, 'utf8');

// 1. REMOVER TODAS AS OCORRÊNCIAS DUPLICADAS DO MENU DE ADMINISTRAÇÃO E AUDITORIA NA SIDEBAR
...
html = html.replace(/<div class="nav-submenu"[^>]*id=["']grupo-admin["'][\s\S]*?<\/div>/g, '');

// 2. INSERIR UM ÚNICO MENU DE ADMINISTRAÇÃO ORGANIZADO ANTES DO FECHAMENTO DA SIDEBAR
const menuAdminUnico = `
          <div class="nav-group" data-group="grupo-admin">
            ...
            <div class="nav-submenu" id="grupo-admin" style="display:none;">
...
// Insere antes do fim da sidebar
if (html.includes('</aside>')) {
  html = html.replace('</aside>', `${menuAdminUnico}\n    </aside>`);
} else if (html.includes('</nav>')) {
  html = html.replace('</nav>', `${menuAdminUnico}\n    </nav>`);
}

// 3. GARANTIR A SEÇÃO HTML DA ABA DE PERFIS E DA ABA DE AUDITORIA NO CORPO (MAIN)
const abaPerfisHTML = `
      <!-- TAB: PERFIS -->
      <section id="tab-perfis" class="tab-content" style="display:none;">
        <div class="section-title-wrap">
          <h2 class="section-title"><span>🛡️</span> Perfis de Acesso e Permissões</h2>
        </div>
          ...
          <table class="data-table">
            ...
            <tbody>
              <tr>
                ...
                <td>Acesso total ao sistema, cadastros, DRE, usuários e auditoria.</td>
                <td><span class="badge-pos">Ativo</span></td>
              </tr>
              <tr>
                <td><strong>Financeiro</strong></td>
                <td>Gestão financeira, DRE e relatórios executivos.</td>
                <td><span class="badge-pos">Ativo</span></td>
              </tr>
              <tr>
                <td><strong>Operador</strong></td>
                <td>Lançamentos operacionais de KM, abastecimentos e acoplamentos.</td>
                <td><span class="badge-pos">Ativo</span></td>
              </tr>
            </tbody>
          </table>
...
if (!html.includes('id="tab-perfis"')) {
  html = html.replace('</main>', abaPerfisHTML + '\n    </main>');
}

// Garante aba auditoria
...
// 4. BLINDAR A FUNÇÃO switchTab NO JAVASCRIPT PARA SUPORTAR PERFEITAMENTE ESSAS ABAS
const overrideSwitchTab = `
...
// Substitui a função switchTab existente pela versão blindada
html = html.replace(/function\s+switchTab\s*\([^)]*\)\s*\{[\s\S]*?\}/, overrideSwitchTab);

// 5. VALIDAR SINTAXE JAVASCRIPT
let erros = 0;
...
if (erros === 0) {
  ...
  console.log('🎉 public/index.html limpo, organizado e validado com 0 erros!');
} else {
