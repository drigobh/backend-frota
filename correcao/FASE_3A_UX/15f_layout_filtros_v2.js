...
// ---- 2) Substituir o HTML do bloco de filtros (versao REAL, com acentos corrompidos) ----
const ANTES = '<div class="bar-controls">' + NL +
'          <div class="bar-controls-left">' + NL +
'            <span class="label-month-select">MÃªs de ReferÃªncia:</span>' + NL +
...
'              <option value="">Todos os veÃculos</option>' + NL +
'            </select>' + NL +
...
'            <button class="btn-action btn-action-primary" onclick="abrirModalLancamento()">+ Novo LanÃ§amento</button>' + NL +
'          </div>' + NL +
'        </div>';
...
'            </div>' + NL +
'            <div class="filtro-item">' + NL +
'              <label for="lanc-filtro-veiculo">Veiculo</label>' + NL +
'              <select id="lanc-filtro-veiculo">' + NL +
'                <option value="">Todos os veiculos</option>' + NL +
'              </select>' + NL +
'            </div>' + NL +
'          </div>' + NL +
...
console.log('  Ocorrencias do bloco: ' + oc);

if (oc !== 1) {
  console.log('  [ERRO] Esperava 1 ocorrencia, achei ' + oc);
  console.log('  Verifique se o HTML mudou de novo.');
  process.exit(1);
}
...
console.log('  1. git add . && git commit -m "feat(ui): filtros compactos em Lancamentos"');
